...
/**
 * 场景：
 * 有时候一般在一个项目中会设计两套账号体系，比如一个电商系统的user表和admin表，在这种情况下，如果两套账号都使用stpUtil类的API
 * 做登录鉴权，那会发生逻辑冲突
 * 解决思路
 * 假如我们的user表和admin表都有一个id=10001的账号，他们对应的登录代码:StpUtil.login(10001)是一样的
 * 那么在StpUtil.getLoginId()获取到的账号id如何区分它是User用户还是Admin用户，可以为他们加一个固定前缀
 * 比如StpUtil.getLogin（"User_" + 10001)StpUtil.login("Admin_" + 10001)，这样确实是可以解决问题的，
 * 但是同样的：你需要在StpUtil.getLoginId()时再裁剪掉相应的前缀才能获取真正的账号id，这样一增一减就让我们的代码变得无比啰嗦
 * 大招：
 * 查看StpUtil类的源码可以看到这个类没有什么代码逻辑，唯一做的就是对成员变量stpLogic的各个API包装一下转发
 *
 */
