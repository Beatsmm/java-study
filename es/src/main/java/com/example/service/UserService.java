...
public class UserService {
    ...
    // 创建索引,可以在CreateIndexRequest中设置索引名称,分片数,副本数,以及Mapping,索引名称为user,分片数number_of_shards为1,副本数为0
    ...
    // 创建文档的时候需要在indexRequest中指定索引名称,id如果不传的话会由ES自动生成,然后传入source
    public Boolean createUserDocument(UserDocument document) throws Exception{
        UUID uuid = UUID.randomUUID();
        document.setId(uuid.toString());
        IndexRequest indexRequest = new IndexRequest(Constant.INDEX)
                .id(document.getId())
                .source(GsonUtil.toString(document),XContentType.JSON);
    ...
    public UserDocument getUserDocument(String id) throws IOException {
        ...
        if (getResponse.isExists()) {
            ...
            result = GsonUtil.fromJson(sourceAsString, UserDocument.class);
        } else {
    ...
    public Boolean updateUserDocument(UserDocument document) throws Exception {
        ...
        updateRequest.doc(GsonUtil.toString(document), XContentType.JSON);
    ...
    private List<UserDocument> getSearchResult(SearchResponse response) {
        ...
        for (SearchHit hit : searchHit) {
            userDocuments.add(GsonUtil.fromJson(hit.getSourceAsString(), UserDocument.class));
        }
